...
- `.github/workflows/ci.yml`: PR と `main` push で `ruff check`、PostgreSQL 付き pytest、Docker build を実行
- `.github/workflows/release.yml`: `v*.*.*` タグ、または手動実行で品質ゲート後に GHCR へ Docker image を publish
- `.github/workflows/fault-tests.yml`: 手動または週次で Docker Compose を起動し、Toxiproxy の障害テストを実行
- `.github/dependabot.yml`: GitHub Actions、uv 依存、Docker image を週次で更新

release workflow は GitHub の `production` environment を使います。
必要に応じて repository settings で required reviewers や deployment branch policy を設定してください。
...
公開される image は `ghcr.io/<owner>/<repo>:v0.1.0`、`sha-<commit>`、
タグリリース時は `latest` です。
